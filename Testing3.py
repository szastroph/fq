...
for j in range(gal_num):
    ...
    # res1[0, j], res[1, j], res[2, j], res[3, j] = obj.shear_est(gal_image, psf, hlr,noise2)[0:4]
        res[0, 25*j+i], res[1,25*j+i], res[2,25*j+ i], res[3,25*j+i] = obj.shear_est(gal_image, psf,noise2)[0:4]
...
if rank == 0:
    print("\nNO. %d \n\n"%ind)
    np.savez("/home/shenzhi/data4/%d_%d.npz"%(gal_num,ind),est_g)
    # x = g1
    # y = est_g[0]
    # y_err = est_g[1]
    # y2 = est_g[2]
